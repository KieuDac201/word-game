...
import { DIFFICULTY_CONFIGS, SPEED_CONFIGS, LANE_COUNT } from "./config";
import { cleanWord, buildSentenceQueue } from "@/lib/core/text";
import {
...
/** Split a sentence into per-word typing targets. */
...
function pickLane(active: FallingSentence[]): number {
  const occupied = new Set(active.filter((s) => s.y < 120).map((s) => s.lane));
...
export function trySpawnSentence(
  ...
  if (currentTime - state.lastSpawnTime < diffConfig.spawnIntervalMs)
    return false;
...
export function handleKeystroke(
  ...
  if (key === " ") {
    ...
    if (
      activeWord.charIndex === 0 &&
      currentTime - state.lastSentenceCompleteTime < 300
    ) {
      return result;
    }
 ...
 */
export function checkVictory(state: GameState, config: GameConfig): boolean {
  if (!config.isCustomSource) return false;
  if (state.sentenceQueue.length > 0) return false;
  if (state.activeSentences.some((s) => !s.completed && !s.dropped))
    return false;
  return true;
}
