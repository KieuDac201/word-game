...
import { getTopMistypedKeys } from "@/lib/core/scoring";
...
export default function PlayPage() {
  ...
  // ─── Config from the game session ──────────────────────────
  const [config] = useState<GameConfig | null>(() => session.loadConfig());
  ...
  if (!config) {
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-white/30 font-[family-name:var(--font-mono)]">
          Loading...
        </div>
      </div>
    );
            ...
            return (
              ...
              >
                {sentence.words.map((word, wIdx) => {
                  ...
                  const isCurrentWord =
                    wIdx === sentence.activeWordIndex && isTargeted;
                  const isLastWord = wIdx === sentence.words.length - 1;
                  const waitingForSpace =
                    isCurrentWord &&
                    word.charIndex >= word.clean.length &&
                    !isLastWord;
                      ...
                      {word.completed ? (
                        ...
                        <>
                          ...
                          {word.raw.length > word.clean.length && (
                            <span
                              className={
                                word.completed ? "char-correct" : "char-pending"
                              }
                            >
            ...
            <div className="glass-card p-8 max-w-lg w-full mx-4 animate-pop-in">
              ...
              <div className="flex flex-col gap-3">
                <button
                  ...
                  style={{
                    background:
                      "linear-gradient(135deg, rgba(0, 240, 255, 0.2), rgba(112, 0, 255, 0.25))",
                    borderColor: "var(--neon-cyan)",
                  }}
                >
                  <span className="text-base">
                    📜 Review All Sentences & Translations (
                    {gameOverStats.sentenceHistory?.length || 0})
                  </span>
                </button>
